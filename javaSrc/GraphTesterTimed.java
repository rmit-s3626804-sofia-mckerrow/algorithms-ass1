...
public class GraphTesterTimed
{
	...
	public static void main(String[] args) {
		...
		OptionParser parser = new OptionParser("f:s:");
		...
		if (options.has("s")) {
			if (options.hasArgument("s")) {
				scenarioFilename = (String) options.valueOf("s");
			}
			else {
				System.err.println("Missing filename argument for -s option.");
				usage(progName);
			}
		}	
		...
		try {
			...
			if (inputFilename != null)
			{
				System.out.println("Loaded base data "+inputFilename+" in time: " + String.valueOf(stopTime-startTime));
